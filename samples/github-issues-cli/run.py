...
def main() -> None:
    runtime = Path(tempfile.mkdtemp(prefix="agora-github-issues-cli-sample-"))
    ...
    agora.install_tool_adapter(InstallToolAdapterInput(adapter_id="github-issues", scope="project"))
    ...
    agora.create_swarm(
        CreateSwarmInput(
            id="delivery",
            objective="Manage work through the existing GitHub CLI",
            create_branch=False,
        )
    )
    for role, actor in (
    ...
    try:
        ...
        raise AssertionError("Unsafe GitHub issue subcommand was unexpectedly accepted")
