...
session. Lifecycle authorization binds the requested mutation to current durable state; session
preparation uses the prospective `CONTEXT.md` digest. The separate launch authorization binds the
exact runtime command and SHA-256 of its materialized `CONTEXT.md`. The resulting
