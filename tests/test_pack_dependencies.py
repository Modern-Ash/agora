...
def test_pack_update_protects_local_modifications(tmp_path: Path, monkeypatch) -> None:
    ...
    audit = workspace.audit_pack_updates(AuditPackUpdatesInput(scope="project", record=True))
    with pytest.raises(ValueError, match="contains local modifications"):
        workspace.apply_pack_update_audit(ApplyPackUpdateAuditInput(id=audit.id, scope="project"))
    assert workspace.show_tool("tracker").version == "1.0.0"

    workspace.apply_pack_update_audit(
        ApplyPackUpdateAuditInput(id=audit.id, scope="project", force=True)
    )
...
def test_audits_catalog_pack_updates_and_records_a_markdown_notification(
    ...
    assert preview.entries[0].registry_scope == "project"
    assert workspace.show_tool("tracker").version == "1.0.0"
    ...
    assert (
        main(
            ["pack", "apply-audit", "--id", recorded.id, "--scope", "project"],
            cwd=workspace.project_root(),
            stdout=output,
        )
        == 0
    ...
    assert workspace.show_tool("tracker").version == "2.0.0"
    assert (Path(recorded.path).with_name("APPLICATION.md")).is_file()
    assert workspace.validate().ok is True
    ...
    assert any(
        issue.code == "pack-update-audit-application.audit-mismatch" for issue in invalid.issues
    )


def test_apply_pack_audit_rejects_state_changed_after_review(tmp_path: Path, monkeypatch) -> None:
    workspace = _workspace(tmp_path, monkeypatch)
    source_v1 = _tool(tmp_path / "first" / "tracker", "tracker", "1.0.0")
    registry_v1 = _registry(tmp_path / "registry-v1", [("tool", source_v1)])
    workspace.install_registry(InstallRegistryInput(source=str(registry_v1), scope="project"))
    ...
    recorded = workspace.audit_pack_updates(AuditPackUpdatesInput(scope="project", record=True))
    manifest = workspace.project_root() / ".agora" / "tools" / "tracker" / "TOOL.md"
    manifest.write_text(
    ...
    with pytest.raises(ValueError, match="audit is stale"):
        workspace.apply_pack_update_audit(
            ApplyPackUpdateAuditInput(id=recorded.id, scope="project")
        )

    assert workspace.show_tool("tracker").version == "1.0.0"
    assert not Path(recorded.path).with_name("APPLICATION.md").exists()
